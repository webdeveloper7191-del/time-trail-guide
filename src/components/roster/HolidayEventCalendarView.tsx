...
interface HolidayEventCalendarViewProps {
  currentDate: Date;
...
export function HolidayEventCalendarView({ 
  currentDate: initialDate, 
  onDateClick,
  onClose 
}: HolidayEventCalendarViewProps) {
              ...
              {selectedDateDetails.events.map(ev => (
                <div key={ev.id} className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    {ev.affectedRooms && ev.affectedRooms.length > 0 && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Rooms: {ev.affectedRooms.join(', ')}
                      </p>
                    )}
                  </div>
                </div>
          ...
          <ScrollArea className="h-48">
            <div className="space-y-2">
              ...
              {monthEvents.map(ev => (
                <div 
                  key={ev.id} 
                  className="flex items-center gap-3 p-2 rounded-lg bg-card border cursor-pointer hover:bg-accent"
