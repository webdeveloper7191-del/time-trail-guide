...
const severityFills: Record<string, string> = { minor: 'hsl(var(--status-approved))', moderate: 'hsl(var(--warning))', critical: 'hsl(var(--destructive))' };
...
const tableColumns: DataTableColumn<CoverageGapRecord>[] = [
  ...
  { key: 'timeSlot', header: 'Time Slot', accessor: (r) => <span className="text-muted-foreground">{r.timeSlot}</span>, sortValue: (r) => r.timeSlot },
  ...
  { key: 'gap', header: 'Gap', align: 'right', sortValue: (r) => r.gap,
    accessor: (r) => <span className={cn('font-medium', r.gap > 0 && 'text-destructive')}>{r.gap > 0 ? `-${r.gap}` : '✓'}</span> },
  { key: 'gapSeverity', header: 'Severity', sortValue: (r) => r.gapSeverity,
    accessor: (r) => <Badge className={cn('text-xs capitalize', severityColors[r.gapSeverity])}>{r.gapSeverity}</Badge> },
  { key: 'reason', header: 'Reason', accessor: (r) => <span className="text-muted-foreground">{r.reason || '—'}</span>, sortValue: (r) => r.reason },
];
...
export function CoverageGapReport() {
  ...
  const chartData = filtered.filter(r => r.gap > 0).map((r) => ({
    name: `${r.area} (${format(parseISO(r.date), 'dd/MM')})`, gap: r.gap, severity: r.gapSeverity,
  }));
          ...
          <CardContent><div className="h-[220px]"><ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
              ...
              <Bar dataKey="gap" name="Staff Gap" radius={[4, 4, 0, 0]}>
                {chartData.map((entry, i) => <Cell key={i} fill={severityFills[entry.severity]} />)}
              </Bar>
            </BarChart>
