...
const tableColumns: DataTableColumn<CasualVsPermanentRecord>[] = [
  ...
  { key: 'permanentHours', header: 'Perm Hrs', type: 'number', accessor: (r) => <span className="font-mono text-xs">{r.permanentHours}h</span>, sortValue: (r) => r.permanentHours, align: 'right' },
  { key: 'casualHours', header: 'Casual Hrs', type: 'number', accessor: (r) => <span className="font-mono text-xs">{r.casualHours}h</span>, sortValue: (r) => r.casualHours, align: 'right' },
  { key: 'permanentCost', header: 'Perm Cost', type: 'number', accessor: (r) => `$${(r.permanentCost / 1000).toFixed(1)}k`, sortValue: (r) => r.permanentCost, align: 'right' },
  { key: 'casualCost', header: 'Casual Cost', type: 'number', accessor: (r) => <span className="text-destructive">${(r.casualCost / 1000).toFixed(1)}k</span>, sortValue: (r) => r.casualCost, align: 'right' },
  { key: 'costPerHourPermanent', header: '$/hr Perm', type: 'number', accessor: (r) => `$${r.costPerHourPermanent}`, sortValue: (r) => r.costPerHourPermanent, align: 'right' },
  { key: 'costPerHourCasual', header: '$/hr Casual', type: 'number', sortValue: (r) => r.costPerHourCasual, align: 'right',
