...
const tableColumns: DataTableColumn<HeadcountRecord>[] = [
  ...
  { key: 'totalHeadcount', header: 'Headcount', accessor: (r) => <span className="font-semibold">{r.totalHeadcount}</span>, sortValue: (r) => r.totalHeadcount, align: 'right' },
  { key: 'fte', header: 'FTE', accessor: (r) => r.fte.toFixed(1), sortValue: (r) => r.fte, align: 'right' },
  { key: 'fteRatio', header: 'FTE Ratio', align: 'right', sortValue: (r) => r.fte / r.totalHeadcount,
    accessor: (r) => {
      ...
      <Card className="border-border/60">
        ...
        <CardContent>
          <AnimatedChartWrapper animKey={animKey}><ResponsiveContainer width="100%" height={260}>
            <BarChart data={byDepartment} cursor="pointer" onClick={(e: any) => { if (e?.activeLabel) applyDrill('department', e.activeLabel); }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
