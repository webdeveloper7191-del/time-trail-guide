...
const tableColumns: DataTableColumn<OnboardingRecord>[] = [
  { key: 'staffName', header: 'Staff', type: 'number', accessor: (r) => <div><span className="font-medium">{r.staffName}</span><span className="block text-[10px] text-muted-foreground">{r.position}</span></div>, sortValue: (r) => r.staffName },
  ...
  { key: 'daysInPipeline', header: 'Days', type: 'date', accessor: (r) => <span className={cn('text-xs font-medium', r.daysInPipeline > 21 ? 'text-destructive' : 'text-foreground')}>{r.daysInPipeline}d</span>, sortValue: (r) => r.daysInPipeline, align: 'right' },
