    ...
    groups: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            type: 'Boolean',
            defaultValue: 'On',
            purpose: 'Allows punches from the mobile app.',
            logic: [
              'When off, mobile punches are rejected and the app shows a "clock disabled by your organisation" state.',
              'GPS capture and geofencing only apply to this channel.',
            ],
            interactions: ['captureGpsOnMobile', 'restrictToGeofence'],
          },
          {
            key: 'timeTracking.captureGpsOnMobile',
            label: 'Capture GPS on mobile',
            type: 'Boolean',
            defaultValue: 'Off',
            purpose: 'Stores latitude/longitude with each mobile punch for audit.',
            logic: [
          ...
          },
          {
            ...
            label: 'Require kiosk photo',
            type: 'Boolean',
            defaultValue: 'Off',
            purpose: 'Captures a photo at shared-device punches to deter buddy punching.',
            logic: [
              'Applies to kiosk/shared-device mode only. If the camera is unavailable the punch is blocked.',
              'Photos are attached to the clock event and surfaced in the reviewer panel next to any buddy-punching flag.',
            ],
            interactions: ['Timesheet Issues → Buddy punching detection uses photo presence as corroborating evidence'],
          },
        ],
      },
      {
    ...
    groups: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            type: 'Enum',
            options: ['Never (unpaid)', 'Always paid', 'Paid when over threshold'],
            ...
            logic: [
              ...
              '"Over threshold": the break is paid only when the shift’s worked minutes exceed paidMealOverMinutesThreshold; otherwise unpaid.',
          ...
          {
            key: 'breaks.paidMealOverMinutesThreshold',
            label: 'Paid meal threshold',
            type: 'Number (minutes)',
            defaultValue: '30',
            purpose: 'The shift length at which meal breaks become paid.',
            logic: [
              'Only used with the "Over threshold" mode; the comparison is against actual worked minutes excluding breaks.',
