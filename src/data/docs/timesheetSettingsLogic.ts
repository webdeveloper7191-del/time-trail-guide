    ...
    groups: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            type: 'Enum',
            options: ['Never (unpaid)', 'Always paid', 'Paid on longer shifts'],
            ...
            logic: [
              ...
              '"Paid on longer shifts": the meal break is paid only when the shift is longer than `paidMealOverShiftHours`; shorter shifts keep the break unpaid.',
            ...
            interactions: ['breaks.paidMealOverShiftHours', 'breaks.paidMealMaxPaidMinutes', 'Awards → paid break provisions', 'Net hours and overtime calculation'],
            edgeCases: ['The settings screen renders a live "Current outcome" sentence describing the resolved rule in plain English, because the four paid-meal fields combine in ways that are hard to read individually.'],
          },
          {
            key: 'breaks.paidMealOverShiftHours',
            label: 'Paid when shift is longer than',
            type: 'Number (hours)',
            defaultValue: '6',
            purpose: 'The shift length at which meal breaks become paid.',
            logic: [
              'Only used with the "Paid on longer shifts" mode. The comparison uses the rostered/actual shift span in hours (worked time plus breaks).',
              'The test is strictly greater than: a shift exactly equal to the threshold is not paid.',
              'Applies per shift, not per week — a split shift is evaluated on each segment.',
            ],
            example: 'Threshold 6h: an 8h shift with a 30-min meal is paid 8h; a 5h shift with a 30-min meal is paid 4.5h.',
          },
          {
            key: 'breaks.paidMealMaxPaidMinutes',
            label: 'Cap paid meal minutes',
            type: 'Number (minutes, 0 = no cap)',
            defaultValue: '0',
            purpose: 'Limits how much of a long meal break the business pays for.',
            logic: [
              '0 means no cap — the whole meal break is paid whenever the paid-meal rule applies.',
              'When set, paid meal minutes are capped at this value and any excess is deducted as unpaid.',
              'Ignored entirely when the resolved mode is "Never (unpaid)".',
            ],
            example: 'Cap 30 min, staff takes a 45-min meal on a paid-meal shift → 30 min paid, 15 min unpaid.',
          },
          {
            key: 'breaks.paidMealCountsTowardHours',
            label: 'Paid meal counts toward hours thresholds',
            type: 'Boolean',
            defaultValue: 'On',
            purpose: 'Controls whether paid meal minutes accrue toward overtime and daily/weekly hour thresholds.',
            logic: [
              'When on, paid meal minutes are included in the hours used for overtime, excessive-hours flags and compliance thresholds.',
              'When off, the minutes are still paid but excluded from those threshold calculations, so a paid meal cannot by itself tip a day into overtime.',
              'Award provisions that explicitly count paid breaks as time worked override an "off" setting.',
            ],
            interactions: ['Compliance Thresholds → max daily/weekly hours', 'Overtime engine', 'Timesheet Issues → excessive hours'],
          },
          {
            key: 'breaks.payInterruptedUnpaidMeal',
            label: 'Pay interrupted unpaid meals',
            type: 'Boolean',
            defaultValue: 'On',
            purpose: 'Compensates staff recalled to work before an unpaid meal break finishes.',
            logic: [
              'Applies only to unpaid meal breaks that were ended early because the staff member returned to work.',
              'When on, the full entitled meal duration is paid — not just the minutes actually taken — matching common award "interrupted meal" provisions.',
              'The interruption is recorded on the entry and raised as an info flag so the approver can see why the break was paid.',
              'When off, only the minutes actually worked are paid and the short break is handled by the short/missed break flag instead.',
            ],
            interactions: ['breaks.flagShortOrMissedBreaks', 'Awards → interrupted meal break provisions'],
          },
        ],
      },
    ],
