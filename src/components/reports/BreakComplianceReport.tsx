...
const tableColumns: DataTableColumn<BreakComplianceRecord>[] = [
  ...
  { key: 'shiftDuration', header: 'Shift', type: 'text', accessor: (r) => `${r.shiftDuration}h`, sortValue: (r) => r.shiftDuration, align: 'right' },
  ...
  { key: 'breakTimings', header: 'Timings', type: 'number', accessor: (r) => <span className="text-xs text-muted-foreground font-mono">{r.breakTimings}</span>, sortValue: (r) => r.breakTimings },
  ...
  { key: 'complianceRateTrend', header: 'Compliance Trend', type: 'sparkline', trendValues: (r: any) => r.complianceRateTrend ?? [], accessor: () => null },
];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-border/60">
        ...
        <CardContent><ReportDataTable key={animKey} columns={tableColumns} data={filtered} rowKey={(_, i) => i} reportId="break-compliance" exportTitle="Break Compliance" /></CardContent>
      </Card>
    </div>
  );
}
