...
export const rosterSRS: ModuleSRS = {
  ...
  businessRules: [
    ...
    // Scheduling Constraints Engine Business Rules
    { id: "BR-RST-036", rule: "Constraint precedence at solve time: ConditionalRule (matched) → LocationOverride → ContractTemplate → Tenant Default. First non-null wins per parameter (sparse override)", rationale: "Predictable, layered configuration — most specific context wins" },
    { id: "BR-RST-037", rule: "HARD constraints prune the solver search space (assignment never considered); SOFT constraints add cost = weight × priorityMultiplier × violationMagnitude to softScore", rationale: "Two-tier scoring (Timefold BendableScore [hardScore, softScore]) — hardScore must equal 0 for a feasible roster" },
    { id: "BR-RST-038", rule: "Priority-to-multiplier mapping: P1-2=10×, P3-4=7×, P5-6=5×, P7-8=3×, P9-10=1×", rationale: "Logarithmic priority scale ensures critical soft constraints dominate cosmetic ones" },
    { id: "BR-RST-039", rule: "Area ratio (US-RST-078) overrides Location ratio (US-RST-077) for any shift bound to that area; both must be satisfied as separate HARD constraints — area scope first, location scope second", rationale: "Regulators enforce ratios at the most granular operational unit (room/department), not site-wide averages" },
    { id: "BR-RST-040", rule: "Contract Template parameters flagged perContract:true override the global constraint default per-employee at solve time; non-perContract parameters remain global only", rationale: "Per-employee limits (max hours, rest, consecutive days) are contractual; thresholds for cost optimisation remain organisational policy" },
    { id: "BR-RST-041", rule: "If multiple Conditional Rules match the same shift context, the rule with lowest priority number wins; ties broken by highest weight", rationale: "Deterministic conflict resolution prevents unstable solver behaviour across re-solves" },
    { id: "BR-RST-042", rule: "Disabling a HARD constraint (enforcement=OFF) requires explicit acknowledgement in UI and is recorded in audit log with actor and reason", rationale: "Hard constraints typically encode legal/safety requirements — accidental disabling could produce non-compliant rosters" },
    { id: "BR-RST-043", rule: "Solver returns INFEASIBLE if hardScore < 0; UI must surface affected shifts, violated constraint codes (H1, H6, etc.) and suggest remediation (relax to SOFT, add staff, split shift)", rationale: "Operators need actionable diagnostics, not just 'no solution'" },
    { id: "BR-RST-044", rule: "Manager-locked assignments (S8 Preserve Pre-Assignments) survive subsequent re-solves; solver can only modify them if employee becomes unavailable (H8 allowOverrideOnUnavailable=true)", rationale: "Trust principle — human curation is respected by the optimiser" },
    // Overnight Shift Handling
    { id: "BR-RST-045", rule: "A shift is anchored to its start date; if endTime <= startTime it is treated as crossing midnight (endsNextDay=true) and 24h is added when computing duration", rationale: "Single canonical anchor prevents double-counting in daily/weekly aggregates and consecutive-day rules" },
    { id: "BR-RST-046", rule: "Daily/weekly hour totals credit ALL minutes of an overnight shift (including post-midnight) to the start date and start date's ISO week", rationale: "Consistent payroll attribution and roster reporting; avoids fragmenting one work session across two reporting buckets" },
    { id: "BR-RST-047", rule: "Min Rest (H6/H11) and shift-duration checks use full ISO timestamps (startDateTime/endDateTime), not same-date HH:mm arithmetic", rationale: "Real elapsed-time evaluation is the only correct basis for fatigue and contractual limits" },
    { id: "BR-RST-048", rule: "Max Consecutive Days (H12/H17) counts overnight shifts by start date only — a 10pm Mon → 6am Tue shift counts as Monday, never as both Mon and Tue", rationale: "Prevents inflated consecutive-day counts that would force unnecessary days off" },
    { id: "BR-RST-049", rule: "Night loading, weekend, and public holiday penalty rates are calculated minute-by-minute against actual clock time, not against the anchor date", rationale: "Award compliance — penalty windows (e.g. 10pm–6am, PH boundaries) are clock-time concepts, independent of which calendar date a shift is logged against" },
    { id: "BR-RST-050", rule: "If clockOut < clockIn on submission, the system auto-rolls clockOut to next day after explicit user confirmation; shifts with elapsed duration >18h trigger R13_OVERNIGHT_EXCESSIVE warning", rationale: "Catches missed clock-out events while supporting legitimate long sleepover/standby shifts" }
  ]
};
